...
/** The SQL half (select, from, cursorColumn) lives in ExportRepository. */
interface DatasetSpec extends ExportSql {
...
const DATASETS: Record<ExportDataset, DatasetSpec> = {
  ...
  purchases: {
    ...
    ...EXPORT_DATASET_SQL.purchases,
  },
};

type Cursor = ExportCursor;

export class ExportService {
  constructor(private readonly exportRepository = new ExportRepository()) {}
  ...
  /** One batch, newest first (keyset pagination — see ExportRepository.fetchBatch). */
  private async fetchBatch(
    spec: DatasetSpec,
    companyId: string,
    cursor: Cursor | null,
  ): Promise<Record<string, unknown>[]> {
    return this.exportRepository.fetchBatch(spec, companyId, cursor, BATCH_SIZE);
  }
